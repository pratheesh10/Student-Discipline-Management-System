...
<html lang="en">
  <head>
    ...
    <title>Student  Discipline System|||Search Students</title>
    <!-- plugins:css -->
...
$sql="SELECT tblstudent.StuID,tblstudent.ID as sid,tblstudent.StudentName,tblstudent.StudentEmail,tblstudent.DateofAdmission,tblstudent.status,tblstudent.principalstatus,tblclass.ClassName,tblclass.Section from tblstudent join tblclass on tblclass.ClassName=tblstudent.StudentClass where tblstudent.StuID like '$sdata%' LIMIT $offset, $no_of_records_per_page";
$query = $dbh -> prepare($sql);
$query->execute();
                        ...
                        <?php }else{ ?>
                          <span class="badge badge-lg badge-danger text-white">Pending</span>
                            ...
                            <?php if ($row->principalstatus == 'accepted') { ?>
                              ...
                              <span class="badge badge-lg badge-info text-white">Accepted</span>
                            <?php }elseif ($row->principalstatusstatus == 'rejected'){ ?>
    
                        <span class="badge badge-lg badge-danger text-white">rejected</span>
                        <?php }else{ ?>
                          <span class="badge badge-lg badge-danger text-white">Pending</span>
                            <?php } ?>
                                                || <a href="search.php?delid=<?php echo ($row->sid);?>" onclick="return confirm('Do you really want to Delete ?');"> <i class="icon-trash"></i></a></div>
                            </td> 
                          </tr><?php 
$cnt=$cnt+1;
} } else { ?>
